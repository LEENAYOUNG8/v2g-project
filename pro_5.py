...
def main():
    ...
    params["pv_annual_kwh"] = st.sidebar.number_input(
        "연간 PV 발전량(kWh)", value=params["pv_annual_kwh"], step=1000, min_value=0
    )
    params["self_use_ratio"] = st.sidebar.slider(
        "PV 자가소비 비율", min_value=0.0, max_value=1.0, value=params["self_use_ratio"], step=0.05
    )
    params["pv_base_price"] = st.sidebar.number_input(
        "PV 기준단가(원/kWh)", value=params["pv_base_price"], step=5, min_value=0
    )
    params["price_cagr"] = st.sidebar.number_input(
        "전력단가 연평균 상승률", value=params["price_cagr"], step=0.001, format="%.3f"
    )
